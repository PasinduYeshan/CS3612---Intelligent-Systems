...
# Input start configuration and goal configuration as cmd inputs
...
iteration_out = open("misplaced-all.txt", "w")
n_puzzle = open("npuzzle.txt", "a")

for i in range(1,101):
    start_configuration_file_name = f"inputs/{str(i)}.txt"
    goal_configuration_file_name = f"goals/{str(i)}.txt"
   ...
   # iteration_out = open("manhattan.txt", "a")
    iteration_out.write(str(iterations))
    iteration_out.write("\n")
    n_puzzle.write(str(size))
    n_puzzle.write("\n")
